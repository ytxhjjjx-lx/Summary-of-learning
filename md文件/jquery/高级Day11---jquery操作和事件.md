...
​	CSS-DOM是针对CSS的操作。主要作用就是获取和设置style对象的各种属性, **一般获取的为第一个元素,设置的为每一个元素**. 让网页呈现出不同的效果。
...
> 三个方法：text()、html()和 val(),  当为多个元素添加方法时,不带参数,则获取第一个元素的内容,带参数则设置每个元素内容.
>
> 分别对应innerText     innerHTML     value
...
> css方法使用总结：
>
> 1. 如果只传入一个字符串，则是获取匹配的第一元素属性的值。css("backgroundColor")获取背景色(rgb颜色)。
...
> 获取这个元素在当前文档的相对偏移(距离页面的左边和上边的距离) 即left和top值, 不考虑margin。其中返回的对象包含两个属性，left和top。(这个值类似pageX和pageY, 距离页面的左边和上边的距离).  而clientX,clientY表示距离浏览器左边和上边的距离
...
> remove()  删除这个节点，绑定的事件会丢失。  
>
> detach() 删除节点，但是绑定的事件还在  
>
> empty()  清空这个节点的所有内容。但是这个节点还在。  
>
> 三个方法可以完成删除节点的操作
...
> a.replaceWith(b): a中的每一个元素都用 b去替换   
>
> a.replaceAll(b) :  用a去把满足b的每一个替换掉
...
### 5.1.1	事件绑定(一般使用on, off方法实现事件绑定与解绑替代下面方法)

>  元素对象.bind(事件，[可选的额外参数], 事件处理函数)
>
>  可选参数2一般省略

**mouseenter, mouseleave不支持冒泡,  mouseover, mouseout支持冒泡**
...
1. hover(f1, f2)用于模拟光标悬停事件，当光标移动到元素上的时候，会触发指定的第一个函数，当光标移除这个元素时，会触发第二个元素,  不支持冒泡,类似于mouseenter, mouseleave
2. 功能1：toggle(f1, f2, ... fn) 用于模拟鼠标连续单击事件，当第一次单击时只需第一个函数，当第二次单击执行第二个函数，...，执行完毕之后再循环来。**功能2：不传任何参数，可以让元素隐藏或显示,  传参表示动画持续时间.**       **但是从1.9版本开始，只剩下了功能2**
...
4. target属性:   获取到导致触发事件的元素,即鼠标点击的元素
5. pageX和pageY属性:    光标(鼠标点击位置)相对于页面顶端和左端的距离。如果有滚动条，还要加上滚动条的距离
6. offsetX和offsetY:           光标(鼠标点击位置)相对于元素顶端和左端的距离
7. clientX和clientY:         光标(鼠标点击位置)相对于浏览器窗口顶端和左端的距离
