...
相当于先描述路径再绘制，先规定路径再规定样式

```javascript
var deg = Math.PI/180;
		context.beginPath(); //循环画多个时必须加这行代码，否则绘制颜色全部一致,以最后设置的颜色为准，相当于属性值覆盖
		context.arc(150,150,100,0,180*deg,false); //放在前面，相当于描述了路径
...
```javascript
  cvs_ctx.beginPath();  
  cvs_ctx.moveTo(50, 50);
  cvs_ctx.lineTo(100, 50);
  cvs_ctx.arc(100, 50-15+2, 20, 0, 360*deg, false);
  cvs_ctx.lineTo(100, 50);
  cvs_ctx.lineTo(150, 50);
  cvs_ctx.lineTo(150, 100);
  cvs_ctx.arc(150+15-2, 100, 20, 0, 360*deg, false);
  cvs_ctx.lineTo(150, 100);
  cvs_ctx.lineTo(150, 150);
  cvs_ctx.lineTo(50, 150);
  cvs_ctx.lineTo(50, 50);
  cvs_ctx.fillStyle = "#fff";
  cvs_ctx.fill();

  cvs_ctx.beginPath();
  cvs_ctx.arc(50, 100, 20, 270*deg, 90*deg, false);
  cvs_ctx.globalCompositeOperation = "xor";
  cvs_ctx.fill();
```
